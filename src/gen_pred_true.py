if __name__ == '__main__':
    ...
    true_W = np.loadtxt(os.path.join(data_path,
                                 ...
                                 str(args.types)+'-'
                                 +str(args.pixels)+'-'
                                 +str(args.index)+'.csv'), delimiter=',').T
    ...
    prior_output, plls = test.alt_max(cluster_init, init_W = true_W,
                                reset=False, prior = prior, iters=args.iters, parallel = True, return_lls=True, start_W = True)
    print("time elapsed",time.time()-start_time)
    
    start_time = time.time()
    noprior_output, nlls = test.alt_max(cluster_init, init_W = true_W,
                                reset=False, prior = None, iters=args.iters, parallel = True, return_lls=True, start_W = True)
    print("time elapsed",time.time()-start_time)
    
    
    
    np.savetxt(os.path.join(data_path,"prior-icurr_S"+"-"+
                                        str(args.types)+"-"+
                                        str(args.pixels)+"-"+
                                        str(args.index)+".csv"), prior_output[0])
    np.savetxt(os.path.join(data_path,"prior-icurr_W"+"-"+
                                        str(args.types)+"-"+
                                        str(args.pixels)+"-"+
                                        str(args.index)+".csv"), prior_output[1])
    
    np.savetxt(os.path.join(data_path,"noprior-icurr_S"+"-"+
                                        str(args.types)+"-"+
                                        str(args.pixels)+"-"+
                                        str(args.index)+".csv"), noprior_output[0])
    np.savetxt(os.path.join(data_path,"noprior-icurr_W"+"-"+
                                        str(args.types)+"-"+
                                        str(args.pixels)+"-"+
                                        str(args.index)+".csv"), noprior_output[1])
    
    np.savetxt(os.path.join(data_path,"prior-ills"+"-"+
                                        str(args.types)+"-"+
                                        str(args.pixels)+"-"+
                                        str(args.index)+".csv"), plls)
    np.savetxt(os.path.join(data_path,"noprior-ills"+"-"+
                                        str(args.types)+"-"+
                                        str(args.pixels)+"-"+
